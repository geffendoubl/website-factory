    ...
    <section className="relative min-h-screen bg-canvas-dark flex flex-col justify-center overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-6 pt-24 pb-20 grid lg:grid-cols-2 gap-14 items-center">
        ...
        <div>
          ...
          {/* Offer — one sentence, no abstraction */}
          <FadeUp delay={0.05}>
            <h1 className="font-display text-4xl md:text-5xl lg:text-[3.25rem] leading-[1.1] tracking-tight text-canvas mb-5">
              ...
              für Ihren Betrieb —
              <br />
              <span className="text-white/50">ab 490€, in Tagen online.</span>
            </h1>
          </FadeUp>
          ...
          {/* Industry examples — early, specific */}
          <FadeUp delay={0.2}>
          ...
          <FadeUp delay={0.28}>
            <div className="flex flex-wrap gap-3 mb-10">
              <button
                ...
                className="px-7 py-3.5 bg-canvas text-ink rounded-full font-semibold text-base hover:bg-canvas/90 transition-colors"
              >
          ...
          <FadeUp delay={0.36}>
            <div className="flex flex-wrap gap-2">
              {trustPills.map((pill) => (
                <span
                  key={pill}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-white/12 bg-white/4 text-white/45 text-xs font-medium"
                >
                  {pill}
                </span>
              ))}
      ...
      {/* Scroll hint — no text, just arrow */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
